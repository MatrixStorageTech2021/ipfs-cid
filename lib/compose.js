...
export const MODULE_KEY = `${PACKAGE_NAME}:ComposeGenerator`;
...
export class ComposeGenerator {
  ...
  mount(generator) {
    ...
    const count = this[GENERATORS].length - 1;
    const generators = this[GENERATORS].slice(0, count);
    let success = false;
    for (const gen of generators) {
      success = generator.init(gen);
      if (success) {
        break;
      }
    }
