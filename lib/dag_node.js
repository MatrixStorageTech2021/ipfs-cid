...
export class DAGNodeGenerator {
  ...
  async generate(target, opts = {}) {
    ...
    const nextNodes = generateNextDAGNodes(rootNodes, parentNodes);
...
/**
 * 计算生成再次计算使用的dagNodes
 * @param {DAGNode[]} roots 起始节点数据
 * @param {AsyncIterable<DAGNode>} nodes 后续节点数据
 */
async function* generateNextDAGNodes(roots, nodes) {

  debug('generateNextDAGNodes %s , %s', roots, nodes);
